...
AuthAPI.interceptors.response.use(
  (res) => res, //accessToken의 만료 시간이 남아있을 경우
  ...
  async (err) => {
    ...
    //refreshToken이 있을 경우, accessToken 재발급
    if (err.response?.status === 401) {
      try {
        ...
        err.config.headers.Authorization = `Bearer ${res.data.accessToken}`; //헤더에 새 accessToken 적용
